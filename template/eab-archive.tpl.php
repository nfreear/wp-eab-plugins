...
<div id="y<?php echo $year; ?>" class="ten">
  ...
  <?php foreach ( $yr_issues as $iss_num => $iss ): ?>
		<li id="i<?= $iss_num ?>">Issue <?= $iss_num ?>:
			<a href="<?php self::lurl( $year, $iss->html_file ); ?>"><?php self::lname( $year, $iss->month ); ?> HTML</a>,
			<a href="<?php self::lurl( $year, $iss->text_file ); ?>"><?php self::lname( $year, $iss->month ); ?> text</a>.
		</li>
	<?php endforeach; ?>
</ul></div>
